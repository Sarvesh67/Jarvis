...
Runs on the host (needs to read ~/.hermes profiles); binds 0.0.0.0 for LAN/Tailscale
access, gated by a DASHBOARD_TOKEN bearer token (see _AuthMiddleware).
"""
import asyncio
import base64
import hmac
...
# Bearer token gating all /api and /ws once the dashboard binds beyond localhost (LAN/Tailscale).
# Empty => fail open (localhost-only legacy behavior). Generated by setup/06_dashboard.sh.
...
class _AuthMiddleware:
    """Gate every /api and /ws request behind DASHBOARD_TOKEN. The app shell (/), static assets,
    and the service worker stay open so the PWA can install before the user is authenticated —
    the token is embedded into the shell HTML at GET / and the frontend replays it on each call.

    Raw ASGI middleware (not BaseHTTPMiddleware) so it sees the *websocket* scope too —
    BaseHTTPMiddleware only dispatches http connections and would let /ws bypass auth entirely."""
    ...
    async def __call__(self, scope, receive, send):
        ...
        path = scope.get("path", "")
        if path in self.OPEN or path.startswith("/static/") or not DASHBOARD_TOKEN:
            return await self.app(scope, receive, send)
        ...
        if auth.startswith("Bearer "):
            provided = auth[7:]
        if not provided:  # browsers can't set headers on WS connect -> ?token=
            qs = parse_qs(scope.get("query_string", b"").decode("latin-1"))
            provided = (qs.get("token") or [""])[0]

        ok = bool(provided) and hmac.compare_digest(
        ...
        if scope["type"] == "websocket":
            ...
            await send({"type": "websocket.close", "code": 1008})  # policy violation -> 403 handshake
            return
        resp = Response(
            status_code=401,
...
# Order matters: last-added wraps outermost. CORS outer handles OPTIONS preflight before auth.
...
@app.get("/sw.js")
def service_worker():
    # Served from root (not /static) so the SW scope is "/" and can control the app shell.
    return FileResponse(
        BASE / "static" / "sw.js",
...
def index():
    ...
    # Embed the bearer token into the shell so the frontend can replay it on /api and /ws calls.
    token_script = f'<script>window.__JARVIS_TOKEN={json.dumps(DASHBOARD_TOKEN)};</script>'
    return html.replace("</head>", token_script + "\n</head>", 1)
