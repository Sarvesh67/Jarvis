...
Everything runs as **sarvesh** (no separate service account). The data/gateway stack binds
`127.0.0.1` only; the **dashboard** binds `0.0.0.0` (token-gated) for phone/remote access.
...
| What | URL | Login |
|------|-----|-------|
| Dashboard (local) | http://127.0.0.1:8080 | `DASHBOARD_TOKEN` (auto-injected when served from `/`) |
| Dashboard (LAN / Tailscale) | http://`<host-ip>`:8080 | same token |
...
## Remote / phone access (PWA)

The dashboard binds `0.0.0.0:8080` and gates every `/api` + `/ws` behind `DASHBOARD_TOKEN`
(in `platform/.env`, generated by `06_dashboard.sh`). The app shell, `/static`, and `/sw.js` stay
open so the PWA can install; the token is embedded into the shell at `GET /` and replayed by the
frontend, so users never type it.

- **Same LAN:** browse to `http://<host-ip>:8080` (`ipconfig getifaddr en0` for the IP).
- **Anywhere:** put host + device on Tailscale (connect via the Tailscale app; IP via
  `/Applications/Tailscale.app/Contents/MacOS/Tailscale ip -4`) and use the Tailscale IP.
- **Install on iPhone:** open the URL in **Safari** → Share → **Add to Home Screen**. Launches
  standalone. The layout is responsive (sidebar becomes a bottom tab bar on phones).
- **Rotate the token:** edit `DASHBOARD_TOKEN` in `platform/.env`, `launchctl kickstart -k
  gui/$(id -u)/com.jarvis.dashboard`, then reload the page once in Safari to pick up the new token.
...
TOKEN=$(grep '^DASHBOARD_TOKEN=' platform/.env | cut -d= -f2-)    # then the gated API:
curl -s -H "Authorization: Bearer $TOKEN" http://127.0.0.1:8080/api/overview | python3 -m json.tool
